...
void get_primary_header_par (char *, int*,  int*,  int*,  int*);
...
static int cc_tbfres (char *keyword);
...
void get_primary_header_par (char *input,
    ...
    if ((status = Get_KeyI(&phdr, "CENTERA2", 0, 0, ycenter)) )
        return;

    if ((status = Get_KeyI(&phdr, "SIZAXIS1", 0, 0, nx)) )
        return;

    if ((status = Get_KeyI(&phdr, "SIZAXIS2", 0, 0, ny)) )
        return;
...
static void tbhdr2image (IRAFPointer tp, Hdr *h) {
	...
	for (i=1; i <= nlines; i++) {
	    ...
	    */
	    ...
	    if (cc_tbfres(keyword))	/* don't copy reserved keywords */
		continue;
